...
for FILE in "$SRC_DIR"/*; do
  ...
  if [[ -f "$FILE" && ( -z "$REGEX" || "$(basename "$FILE")" =~ $REGEX ) ]]; then
    ...
    # Verifica se o ficheiro já existe e se é mais recente que o backup
  ...
  # Verifica se é um diretório
    ...
    echo "While backuping $FILE: $ERRORS Errors; $WARNINGS Warnings; $FILE_UPDATE Updated; $FILE_COPY Copied; $FILE_DELETED Deleted"
